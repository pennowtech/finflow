...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        def add_nav(text, page_index, fa_icon_name):
            ...
            btn.setIcon(qta.icon(f"fa5s.{fa_icon_name}", color="#8f97c5"))
    ...
    def create_dashboard_page(self):
        ...
        grid.addWidget(cardBalance, 0, 0, 1, 2)
        grid.addWidget(cardRevenue, 0, 2)
        grid.addWidget(cardSaving, 0, 3)

        # Row 1 + 2: Popular categories, expenses, frequency, history
        self.category_items = [
            ("Housing", "$ 1,200.00 · Last Week"),
            ("Food", "$ 460.00 · Last Week"),
            ("Entertainment", "$ 320.50 · Last Week"),
            ("Transportation", "$ 249.50 · Last Week"),
            ("Other", "$ 140.00 · Last Week"),
            ("Subscriptions", "$ 60.00 · Last Week"),
        ]
        ...
        grid.addWidget(categories, 1, 0, 2, 2)
        grid.addWidget(expenses, 1, 2, 1, 2)
        grid.addWidget(freqCard, 2, 2)
        grid.addWidget(history, 2, 3)
        grid.addWidget(upcomingCard, 3, 0, 1, 2)
        grid.addWidget(goalsCard, 3, 2, 1, 2)
